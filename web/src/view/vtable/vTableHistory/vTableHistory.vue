  ...
  <el-input v-model="searchInfo.remark" placeholder="搜索条件" />
    ...
    <template #default="scope">{{ formatBoolean(scope.row.active) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updateVTableHistoryFunc(scope.row)">编辑</el-button>
...
import {
  ...
  createVTableHistory,
  deleteVTableHistory,
  deleteVTableHistoryByIds,
  updateVTableHistory,
  findVTableHistory,
  getVTableHistoryList
...
const getTableData = async() => {
  const table = await getVTableHistoryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteVTableHistoryByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateVTableHistoryFunc = async(row) => {
    const res = await findVTableHistory({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteVTableHistoryFunc = async (row) => {
    const res = await deleteVTableHistory({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
