...
<template>
  ...
  <el-select v-model="searchInfo.sourceID" filterable placeholder="请选择文件源ID" :clearable="true">
  ...
  <el-input v-model.number="searchInfo.index" placeholder="搜索条件" />
</el-form-item>
  ...
  <el-input v-model="searchInfo.description" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="文件源ID" prop="sourceID" width="120">
    <template #default="scope">
        <span>{{ filterDataSource(dataSource.sourceID,scope.row.sourceID) }}</span>
    </template>
</el-table-column>
            <el-table-column align="left" label="序列" prop="index" width="120" />

            <el-table-column align="left" label="原始字段值" prop="key" width="120" />

            <el-table-column align="left" label="字段名称" prop="name" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updateFileFieldFunc(scope.row)">编辑</el-button>
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="detailShow" :show-close="true" :before-close="closeDetailShow" title="查看">
            <el-descriptions :column="1" border>
                    <el-descriptions-item label="文件源ID">
    <template #default="scope">
    ...
    {{ detailForm.index }}
                    ...
                    <el-descriptions-item label="原始字段值">
    {{ detailForm.key }}
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getFileFieldList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteFileFieldByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateFileFieldFunc = async(row) => {
    const res = await findFileField({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteFileFieldFunc = async (row) => {
    const res = await deleteFileField({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
