...
<template>
  ...
  <el-select v-model="searchInfo.sourceID" filterable placeholder="请选择数据源ID" :clearable="true">
  ...
  <el-input v-model="searchInfo.remark" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column label="样式配置" prop="config" width="200">
    <template #default="scope">
        [JSON]
    </template>
</el-table-column>
            <el-table-column label="默认数据" prop="defaultData" width="200">
    <template #default="scope">
        [JSON]
    </template>
</el-table-column>
    ...
    <template #default="scope">{{ formatBoolean(scope.row.active) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button v-auth="btnAuth.edit" type="primary" link icon="edit" class="table-button" @click="updatePdfmeHistoryFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.version }}
                    ...
                    <el-descriptions-item label="样式配置">
    {{ detailForm.config }}
</el-descriptions-item>
                    <el-descriptions-item label="默认数据">
    {{ detailForm.defaultData }}
</el-descriptions-item>
...
<script setup>
import {
    getPdfmeHistoryDataSource,
  createPdfmeHistory,
  deletePdfmeHistory,
  deletePdfmeHistoryByIds,
  updatePdfmeHistory,
  findPdfmeHistory,
  getPdfmeHistoryList
} from '@/api/pdfme/pdfmeHistory'
...
const formData = ref({
  ...
  const getDataSourceFunc = async()=>{
    const res = await getPdfmeHistoryDataSource()
    if (res.code === 0) {
...
const getTableData = async() => {
  const table = await getPdfmeHistoryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deletePdfmeHistoryByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updatePdfmeHistoryFunc = async(row) => {
    const res = await findPdfmeHistory({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deletePdfmeHistoryFunc = async (row) => {
    const res = await deletePdfmeHistory({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createPdfmeHistory(formData.value)
                  break
                case 'update':
                  res = await updatePdfmeHistory(formData.value)
                  break
                default:
                  res = await createPdfmeHistory(formData.value)
                  break
